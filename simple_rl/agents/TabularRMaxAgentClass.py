...
#        print(self.action_map)
    ...
    def reset_thres(self, new_thres):
        '''Reset the threshold (to be larger)'''
        self.s_a_threshold = new_thres
        self.known_map = np.zeros((len(self.states), len(self.actions)))
...
#        print("update", state, action)
        if state != None and action != None:
            ...
            if self.n_s_a[s_id][a_id] >= self.s_a_threshold and self.known_map[s_id][a_id] == 0:
                self.known_map[s_id][a_id] = 1
#                print("known", s_id, a_id, self.n_s_a_s[s_id][a_id]/self.n_s_a[s_id][a_id])
                if not self.greedy:
                    self.q = self.planning()
#                    for s in self.states:
#                        print("policy", s, self.get_policy_action(s,self.q))
...
#                return 0.0
    ...
    def incorporate(self, group: MDPGroup):
        '''
        Incorporate the counts from the group
        '''
