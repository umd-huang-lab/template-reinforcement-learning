...
from simple_rl.agents import QLearningAgent, RandomAgent, TabularRMaxAgent, PatternLearningAgent
...
#            print(mdp.lava_locs)
            ...
            if new_task > 0:
                # Reset the agent.
                if type(agent)==PatternLearningAgent or type(agent)==TabularRMaxAgent:
                    agent.reset_with_states(mdp.states, mdp.state_map)
                else:
                    agent.reset()
            # Run the agent.
            hit_terminal, total_steps_taken, _ = run_single_agent_on_mdp(agent, mdp, episodes, steps, experiment, verbose, track_disc_reward, reset_at_terminal, resample_at_terminal)

            if new_task == samples-1:
                # Reset the agent.
                agent.reset()
